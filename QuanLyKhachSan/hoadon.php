...
// PHẦN 1: XỬ LÝ THANH TOÁN (POST REQUEST)
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'pay') {
    $madp = (int)$_POST['madp'];
    $maphong = $conn->real_escape_string($_POST['maphong']);
    $userid = $_SESSION['user_id'] ?? 1;

    // 1. Lấy thông tin phòng và khách hàng
    $sql_dp = "SELECT dp.*, p.MaLoai, kh.MaKH FROM DatPhong dp 
               JOIN Phong p ON dp.MaPhong = p.MaPhong 
               JOIN KhachHang kh ON dp.MaKH = kh.MaKH WHERE dp.MaDP = $madp";
    $dp_query = $conn->query($sql_dp);
    
    // Thoát sớm (Early Return) nếu không tìm thấy đơn
    ...
    // 2. Tính số ngày ở (Dựa trên check-out thực tế hoặc dự kiến)
    ...
    // 3. Tính tiền phòng & Tiền dịch vụ
    $gia_res = $conn->query("SELECT GiaPhong FROM LoaiPhong WHERE MaLoai = {$dp['MaLoai']}");
    $gia = $gia_res ? (int)$gia_res->fetch_assoc()['GiaPhong'] : 0;
    $tienphong = $songay * $gia;

    $dv_res = $conn->query("SELECT SUM(ThanhTien) as Total FROM SuDungDichVu WHERE MaDP = $madp");
    $tiendv = $dv_res ? (int)$dv_res->fetch_assoc()['Total'] : 0;
    $tongtien_tam = $tienphong + $tiendv;

    // 4. Áp dụng Khuyến mãi & Voucher
    $vip_giam = ($songay >= 2) ? round($tongtien_tam * 0.10) : 0; // Giảm 10% nếu ở từ 2 ngày
    $voucher_giam = 0;
    $mavc = $dp['MaVC'];
    $vc_data = null; // Biến tạm lưu data voucher hợp lệ

    // 4.1 Kiểm tra Voucher nhập tay hoặc có sẵn
    ...
    if ($voucher_code_input) {
        ...
        if ($vc_res && $vc_res->num_rows > 0) $vc_data = $vc_res->fetch_assoc();
    } elseif ($mavc) {
        $v_res = $conn->query("SELECT * FROM Voucher WHERE MaVC = $mavc LIMIT 1");
        if ($v_res && $v_res->num_rows > 0) $vc_data = $v_res->fetch_assoc();
    }

    // 4.2 Tính tiền giảm từ Voucher (Gom chung logic để code gọn hơn)
    if ($vc_data && $tongtien_tam >= (float)($vc_data['GiaTriToiThieu'] ?? 0)) {
        $voucher_giam = ($vc_data['LoaiGiam'] == 'phantram') 
            ? round($tongtien_tam * $vc_data['GiaTriGiam'] / 100) 
            : (float)$vc_data['GiaTriGiam'];
        
        $mavc = $vc_data['MaVC']; // Cập nhật lại ID voucher để dùng bên dưới

        // Trừ lượt sử dụng voucher
        $conn->query("UPDATE Voucher SET SoLanDaDung = SoLanDaDung + 1 WHERE MaVC = $mavc");
        if (($vc_data['SoLanDaDung'] + 1) >= $vc_data['GioiHanDung']) {
            $conn->query("UPDATE Voucher SET TrangThai = 'inactive' WHERE MaVC = $mavc");
        }
    }

    // 5. Chốt tổng tiền & Lưu vào DB
    ...
    // 6. Tự động tặng Voucher "Khách hàng thân thiết" (Mỗi 2 lần thanh toán)
    ...
    // 7. Tạo Hóa Đơn & Gửi Email
    $nv_query = $conn->query("SELECT MaNV FROM NhanVien WHERE MaTK = $userid");
    $manv = ($nv_query && $nv_query->num_rows > 0) ? $nv_query->fetch_assoc()['MaNV'] : 'NULL';

    $conn->query("INSERT INTO HoaDon (MaDP, MaNV, TienPhong, TienDichVu, TongTien, GiamGiaThanhVien) 
                  VALUES ($madp, $manv, $tienphong, $tiendv, $tongtien, $sotien_giam)");

    $mail_info = $conn->query("SELECT HoTen, Email FROM KhachHang WHERE MaKH = $makh")->fetch_assoc();
    if ($mail_info && !empty($mail_info['Email'])) {
        $invoice_data = [
            ...
            'tong_tien' => $tongtien,
            'so_ngay' => $songay,
            'ten_voucher' => $vc_data['TenVoucher'] ?? ''
        ];
        sendInvoiceEmail($mail_info['Email'], $mail_info['HoTen'], $invoice_data);
    }

    // Hoàn tất, chuyển hướng về trang hóa đơn
    header("Location: hoadon.php");
    exit;
}

// ==========================================
// PHẦN 2: LOAD GIAO DIỆN (VIEW)
// ==========================================
require_once 'include/header.php';

// 1. Load thông tin đơn phòng đang xử lý thanh toán
$booking = null;
$dichvu_used = [];
$vip_cnt = 0;

if ($room_id) {
    $b_res = $conn->query("SELECT dp.*, kh.HoTen, kh.MaKH, p.MaLoai FROM DatPhong dp 
                           JOIN KhachHang kh ON dp.MaKH = kh.MaKH 
                           JOIN Phong p ON dp.MaPhong = p.MaPhong 
                           WHERE dp.MaPhong = '$room_id' 
                           AND dp.TrangThai IN ('Đang ở', 'Đã thanh toán (Online)', 'Đã xác nhận', 'Chờ xác nhận') 
                           ORDER BY dp.MaDP DESC LIMIT 1");
    if ($b_res && $b_res->num_rows > 0) {
        $booking = $b_res->fetch_assoc();
        
        // Lấy dịch vụ đã dùng
        $dv_rs = $conn->query("SELECT sd.*, dv.TenDV FROM SuDungDichVu sd JOIN DichVu dv ON sd.MaDV = dv.MaDV WHERE sd.MaDP = {$booking['MaDP']}");
        if ($dv_rs) { while($r = $dv_rs->fetch_assoc()) $dichvu_used[] = $r; }

        // Tính số lần VIP cho giao diện
        $vip_check = $conn->query("SELECT COUNT(*) as cnt FROM DatPhong WHERE MaKH = {$booking['MaKH']} AND TrangThai IN ('Đã thanh toán','Đã thanh toán (Online)')");
        $vip_cnt = $vip_check ? (int)$vip_check->fetch_assoc()['cnt'] : 0;
    }
}

// 2. Load danh sách Lịch sử hóa đơn
$hoadon_res = $conn->query("SELECT hd.*, dp.MaPhong, kh.HoTen 
                            FROM HoaDon hd 
                            JOIN DatPhong dp ON hd.MaDP = dp.MaDP 
                            JOIN KhachHang kh ON dp.MaKH = kh.MaKH 
                            ORDER BY hd.NgayLanhToan DESC LIMIT 50");
?>
    ...
    if ($booking['MaVC']) {
        ...
        if ($v_res && $v_res->num_rows > 0) {
            ...
            if ($tongtien_tam >= (float)($vc['GiaTriToiThieu'] ?? 0)) {
                $voucher_giam = ($vc['LoaiGiam'] == 'phantram') ? round($tongtien_tam * $vc['GiaTriGiam'] / 100) : (float)$vc['GiaTriGiam'];
...
<div class="card p-4 border-0 shadow-sm border-top border-warning border-4 mb-4">
    <h4 class="fw-bold text-warning mb-4">Xác Nhận Trả Phòng: <?= htmlspecialchars($room_id) ?></h4>
    <div class="row">
        <div class="col-md-6">
    ...
    <div class="d-flex justify-content-between align-items-center mt-3">
        ...
        <form method="POST">
            ...
            <input type="hidden" name="maphong" value="<?= htmlspecialchars($room_id) ?>">
            <div class="mb-3">
...
<div class="alert alert-info mb-4">
    <i class="fa-solid fa-info-circle me-2"></i>Phòng <strong><?= htmlspecialchars($room_id) ?></strong> hiện không có khách đang ở hoặc đã được thanh toán.
</div>
<?php endif; ?>
        ...
        <table class="table table-hover align-middle table-stackable">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td data-label="Phòng">Phòng <?= htmlspecialchars($row['MaPhong']) ?></td>
                ...
                <?php endwhile; ?>
                <?php else: ?>
                <tr><td colspan="9" class="text-center py-5 text-muted"><i class="fa-solid fa-box-open fs-1 mb-3 d-block"></i>Chưa có hóa đơn nào.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
...
function printHoaDon(row) {
    ...
    <div class="invoice-print">
        ...
        ${parseInt(row.GiamGiaThanhVien) > 0 ? `<div class="inv-discount"><span>🎁 Giảm Giá/Voucher:</span><span>- ${gg} ₫</span></div>` : ''}
